...
def test_run_tools_completes_after_tool_call(httpx_mock: HTTPXMock):
    ...
    result = aai.LLMGateway().chat.completions.run_tools(
        model="claude-sonnet-5",
        messages=messages,
...
def test_run_tools_sends_chat_message_shaped_items(httpx_mock: HTTPXMock):
    # Given the same two-round mock
    httpx_mock.add_response(
        url=COMPLETIONS_URL,
        method="POST",
        status_code=httpx.codes.OK,
    ...
    # When running the tool loop
    aai.LLMGateway().chat.completions.run_tools(
        model="claude-sonnet-5",
        messages=messages,
        tools=[{"type": "function", "function": {"name": "get_weather"}}],
        functions={"get_weather": lambda city: {"forecast": "sunny"}},
    )

    # Then the follow-up request replays the tool call and its output as plain chat messages
    import json as _json

    second_request_body = _json.loads(httpx_mock.get_requests()[1].read())
    replayed = second_request_body["messages"][1:]
    assert replayed[0]["role"] == "assistant"
    assert replayed[0]["tool_calls"][0]["id"] == "call_1"
    assert replayed[0]["tool_calls"][0]["function"]["name"] == "get_weather"
    assert replayed[1]["role"] == "tool"
    assert replayed[1]["tool_call_id"] == "call_1"
    assert _json.loads(replayed[1]["content"]) == {"forecast": "sunny"}

    # And the caller's messages list now holds the complete transcript
    assert messages[-1] == {"role": "assistant", "content": "Hello there."}


def test_run_tools_raises_on_unknown_tool(httpx_mock: HTTPXMock):
...
def test_run_tools_raises_after_max_rounds(httpx_mock: HTTPXMock):
...
def test_run_tools_rejects_streaming():
...
def test_chat_completions_stream_get_final_message(httpx_mock: HTTPXMock):
    ...
    stream = aai.LLMGateway().chat.completions.create(
        model="claude-sonnet-5",
        messages=[{"role": "user", "content": "Hi"}],
        stream=True,
    )
    ...
    # Then it still yields the raw chunks (unchanged behavior)...
    assert len(chunks) == 2
    assert all(isinstance(c, aai.LLMGatewayCompletionChunk) for c in chunks)

    # ...and the accumulator has the assembled text, usage, and finish_reason
