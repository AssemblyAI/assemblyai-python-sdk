...
from assemblyai.prerecorded.v2 import async_api
from assemblyai.prerecorded.v2._base import _BaseTranscriber, _BaseTranscript
from assemblyai.prerecorded.v2.async_client import _upload_request
...
async def test_both_concurrency_models_share_one_base():
    ...
    assert issubclass(aai.AsyncTranscriber, _BaseTranscriber)
    assert issubclass(aai.Transcript, _BaseTranscript)
    assert issubclass(aai.Transcriber, _BaseTranscriber)


async def test_canonical_module_path_exports_both_transcribers():
    """`prerecorded/v2` is the canonical path for both concurrency models."""
    from assemblyai.prerecorded.v2 import AsyncTranscriber, Transcriber

    assert AsyncTranscriber is aai.AsyncTranscriber
    assert Transcriber is aai.Transcriber
