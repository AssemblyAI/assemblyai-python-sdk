...
class SyncTranscriptionConfig(BaseModel):
    ...
    Only `prompt` and `word_boost` shape the transcript; `sample_rate` and
    `channels` are required only for raw PCM audio (WAV carries them in its
    header). `model` selects the sync speech model and is sent as the
    `X-AAI-Model` routing header, not in the request body.
    """
