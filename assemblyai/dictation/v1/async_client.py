...
class AsyncDictationTranscriber:
    ...
    Same input types (path/bytes/file object — no URLs), same
    `DictationConfig`, same `DictationResponse` and `DictationError`, with
    `transcribe()` and `warm()` as coroutines. Use it in asyncio code
    (FastAPI, aiohttp, voice agents), where `DictationTranscriber.transcribe()`
    would block the loop and `transcribe_async()`'s `concurrent.futures.Future`
    is not awaitable.
    ...
    Example:
        ...
        async def main():
            async with aai.AsyncDictationTranscriber() as transcriber:
                result = await transcriber.transcribe("./note.wav")
    ...
    async def transcribe(
        ...
        audio, filename, content_type = await _run_in_thread(
            _resolve_audio, data, config, type(self).__name__
        )
    ...
    async def warm(self) -> bool:
        ...
        distant client, can rival the transcription itself. Awaiting `warm()`
        as soon as you know audio is coming — typically while the clip is
        still being recorded, e.g. via `asyncio.create_task(transcriber.warm())`
        — spends that setup concurrently: the next `transcribe()` reuses the
